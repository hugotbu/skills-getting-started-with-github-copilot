      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = participants.length
          ? participants
              .map(
                (email) =>
                  `<li><span>${email}</span><button type="button" class="delete-participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Unregister ${email}" title="Unregister participant">&#128465;</button></li>`
              )
              .join("")
